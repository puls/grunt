...
function loadTasks(tasksdir) {
  try {
    var files = grunt.file.glob.glob('*' + grunt.file.glob.requireable(), {cwd: tasksdir, maxDepth: 1});
...
// Guess at what the gruntfile might be, perhaps "grunt.js" or maybe
// "grunt.coffee" or something else that node is supporting through
// the magic of third-party extensions.
function findGruntfile() {
  var gruntfile = grunt.file.findup(process.cwd(), 'grunt.js');
  if (gruntfile) {
    return gruntfile;
  }
  
  // If there's no grunt.js file, try loading all of the top-level project's
  // modules, to see if one of them adds support for another language to Node.
  var modules = grunt.file.glob.glob('node_modules/*', {cwd: process.cwd()});
  modules.forEach(function (module) {
    require(path.basename(module));
  });
  return grunt.file.findup(process.cwd(), 'grunt' + grunt.file.glob.requireable());
}
...
task.init = function(tasks, options) {
  ...
  var gruntfile = allInit ? null : grunt.option('config') || findGruntfile();
